...
<<<<<<< HEAD
					<c:forEach var="report" items="${ reportList }">
						<tr>
							<td scope="row"><a
								href="updateReport.do?r_seq=${ report.getR_seq() }">${ report.getR_seq() }</a></td>
							<td>${ report.r_rs1 }</td>
							<td>${ report.r_rs2 }</td>
							<td>${ report.r_rs3 }</td>
							...
							<td align="center"><a
								href="deleteReport.do?seq=${ report.getR_seq() }"
	...
	<div class="container mt-3">
		<div class="row align-items-start">
			...
			<a href="logout.do" class="col-1 btn btn-primary me-2">로그아웃</a> <a
				href="user/user_insert.jsp" class="col-1 btn btn-primary">사용자등록</a>

			<ul class="col pagination justify-content-center">
				<c:set var="cp" value="${ pageInfo.getCurrentPage() }" />

				<c:if test="${ pageInfo.getStartPage() != 1 }">
					<li class="page-item"><a href="getReportList.do?p=1"
						class="page-link"><i class="fas fa-fast-backward"></i></a></li>
					<li class="page-item"><a
						href="getReportList.do?p=${ pageInfo.getStartPage() - 10 }"
				...
				<c:forEach var="page" begin="${ pageInfo.getStartPage() }"
					...
					<li class="page-item ${ (cp==page) ? 'active' : ''}"><a
						href="getReportList.do?p=${page}" class="page-link">${page}</a></li>
				</c:forEach>

				<c:if test="${ pageInfo.getEndPage() < pageInfo.getTotalPages() }">
					<li class="page-item"><a
						href="getReportList.do?p=${ pageInfo.getEndPage() + 1 }"
						class="page-link"><i class="fas fa-forward"></i></a></li>
					<li class="page-item"><a
						href="getReportList.do?p=${ pageInfo.getTotalPages() }"
