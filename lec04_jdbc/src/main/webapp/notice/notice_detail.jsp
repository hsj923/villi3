...
<header>
	...
	<div class="container mt-3" align="center">
		<form action="updateNotice.do" method="post">
			<input name="noti_seq" type="hidden" value="${notice.noti_seq}" />
			
			<div class="input-group mb-3">
  				<span class="input-group-text" id="desc_title"><i class="fas fa-address-book"></i></span>
  				<input type="text" class="form-control" name="title" value="${ notice.noti_title }" disabled>
			</div>
			
			<div class="input-group mb-3">
			  <span class="input-group-text"><i class="fas fa-clipboard"></i></span>
			  <textarea class="form-control"  name="noti_content" rows="20" disabled>${ notice.noti_content }</textarea>
			</div>	
			
			<div class="input-group mb-3">
  				<span class="input-group-text" id="desc_title"><i class="fas fa-calendar"></i></span>
  				<input type="text" class="form-control" name="date" value="${notice.noti_regDate }" disabled>
			</div>
			...
			<a href="notice/notice_delete.jsp" class="btn btn-danger"  onclick="deleteNotice()">글삭제</a>
			<a href="notice/notice_list" class="btn btn-primary">글목록</a>
		</form>			
	</div>
	...
	<script>
		function deleteNotice() {
			if(confirm("공지를 삭제하겠습니까?")) {
		    	self.location.href = "deleteNotice.do?noti_title=${ notice.noti_title }";
		    }
		}
	</script>
</body>
