...
<<<<<<< HEAD
					<c:forEach var="user" items="${ userList }">
						<tr>
							...
							<td>${ user.getRole() }</td>
							<td align="center"><a
								href="deleteUser.do?id=${ user.getId() }" class="btn btn-danger"><i
		...
		<div class="row align-items-start">
			...
			<ul class="col pagination justify-content-center">
				...
				<c:if test="${ pageInfo.getStartPage() != 1 }">
					<li class="page-item"><a href="getUserList.do?p=1"
						class="page-link"><i class="fas fa-fast-backward"></i></a></li>
					<li class="page-item"><a
						href="getUserList.do?p=${ pageInfo.getStartPage() - 10 }"
				...
				<c:forEach var="page" begin="${ pageInfo.getStartPage() }"
					...
					<li class="page-item ${ (cp==page) ? 'active' : ''}"><a
						href="getUserList.do?p=${page}" class="page-link">${page}</a></li>
				</c:forEach>

				<c:if test="${ pageInfo.getEndPage() < pageInfo.getTotalPages() }">
					<li class="page-item"><a
						href="getUserList.do?p=${ pageInfo.getEndPage() + 1 }"
						class="page-link"><i class="fas fa-forward"></i></a></li>
					<li class="page-item"><a
						href="getUserList.do?p=${ pageInfo.getTotalPages() }"
