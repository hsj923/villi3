...
<<<<<<< HEAD
		...
		<form action="updateUser.do" method="post">
			...
			<div class="input-group">
				<input type="checkbox" class="input-group-text" name="role"
					id="role" ${ user.getRole() == "ADMIN" ? "checked" : ""}> <label
					class="form-check-lable ms-sm-2" for="role">관리자</label>
			</div>

			<input type="hidden" name="id" id="id" class="form-control"
				value="${ user.getId() }">
			<button type="submit" class="btn btn-primary text-white">
				<b>"${ user.getId() }"</b>를 수정하겠습니까?
			</button>
		</form>
	</div>
</body>
